 ...
 */
@WebServlet({ "/SearchServlet", "/search.do" })
public class SearchServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 * 클라이언트 요청 URL
		 *  http://localhost/sevletSite/seacrh.do?searchkeyword=java
		 */
	 ...
	 */
		...
		if(searchkeyword==null||searchkeyword.equals("")) {
			/*
			 * search keyword가 null인 경우 ==> http:/localhost/servletSite/search.do
			 * search keyword가 ""인 경우 ==> http:/localhost/servletSite/search.do?searchkeyword=
			 */
			//검색어를 입력하지 않은 경우 재미있는 지식결과를 랜덤하게 보여드립니다.
			out.println("검색어를 입력하지 않은 경우 재미있는 지식결과를 랜덤하게 보여드립니다.<hr>");
			return;
		}
		/*
		 * 2. 검색업무 실행(DB에서 검색)
